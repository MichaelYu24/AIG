...
const COIN_NAME = 'AIGBlock browser';
...
echo '<title>' . COIN_NAME . ' block explorer &middot; AIG Ace v' . ACE_VERSION . "</title>\n";
...
<body>
<h1 class="search_logo"><a><img src="/public/logo.png" alt="Abe logo" /></a> AIG Block browser </h1>
<h3 class="search_logo"><p style="position: absolute;top: 50px;right: 20px;"><a href="/index.php">Home</a>&nbsp;&nbsp;&nbsp;&nbsp;Language selection：<select onChange="window.location=this.value;">
...
<div class="list">
 <div>
    <input type='text' id='page' size="64" class="search" placeholder="You can enter the height of a block, such as 1"><input type='button' onclick='jump();' class="search_btn" value='搜索'>
    <br/>
    <br/>
  </div>
  <table>
    <tr>
      <td>Latest block:<a href="?<?php echo @$ace['num_blocks'];?>"><?php echo @$ace['num_blocks'];?></a></td>
      <td>Whole net force: <?php echo @$ace['hashrate_mhps'];?></td>
      <td>difficulty:<?php echo $diff;?></td>
      <td>Currency Total:<?php echo @$ace['totalcoin'];?></td>
    </tr>
  </table>
<?php
...
?>
  ...
  <table>
    <tr>
      ...
      <td><b>Block data</b></td>
      <td><b>Output value</b></td>
    </tr>
    <?php
    foreach( $ace['blocks'] as $block ){
		$txid = '<option value="">Select View number</option>';
	...
	$newer = $query < $ace['num_blocks'] ? '<a href="?' . ( $ace['num_blocks'] - $query >= BLOCKS_PER_LIST ? $query + BLOCKS_PER_LIST : $ace['num_blocks'] ) . '">&lt; Previous page</a>' : '&lt; Previous page';
    $older = $query - count( $ace['blocks'] ) >= 0 ? '<a href="?' . ( $query - BLOCKS_PER_LIST ) . '">next page &gt;</a>' : 'next page &gt;';
	$older1 = $query - count( $ace['blocks'] ) >= 0 ? '<a href="?10' . '">First page</a>' : 'First page';
	?>    
    <tr>
    ...
    <tr>
      <td colspan="7" class="urgh">Power by AIG Copyright 2018</td>
    </tr>
  </table>  
<?php
}else if(@$_GET['tx'])
...
Transaction data:<?php echo $ace['txid'];?><br />
Block data:<?php echo $ace['blockhash'];?><br />
Trading time:<?php echo date("Y-m-d H:i:s",$ace['time']);?><br />
Generation time:<?php echo date("Y-m-d H:i:s",$ace['blocktime']);?><br />
Confirm:<?php echo $ace['confirmations'];?><br />
Public key signature:<?php
...
Transaction signature:	<?php
			...
			?><br />
Based on the last two lines of data:
			<?php
				foreach($ace['vout'] as $tx){
					echo "<br />Receive address：".@$tx['scriptPubKey']['addresses'][0]." &nbsp;&nbsp;Receiving quantity：".@$tx['value']."&nbsp;&nbsp;";
				}	
			?>
</p>
</td></tr></table>
